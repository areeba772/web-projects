...
router.get('/', async (req, res) => {
  try {
    const items = await FoundItem.find().sort({ createdAt: -1 }).populate('reportedBy', 'name email phone');
    res.status(200).json({
      success: true,
      count: items.length,
      data: items
...
router.get('/:id', async (req, res) => {
  try {
    const item = await FoundItem.findById(req.params.id).populate('reportedBy', 'name email phone');
    if (!item) {
